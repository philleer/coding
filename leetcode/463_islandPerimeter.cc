...
public:
    /*=========================================================================
     * Description:  Given a map in form of 2-d integer grid where 0 represents
     *		land and 0 represents water.  Grid cells are connected horizontally
     *		or vertically(not diagonally). The grid is completely surrounded by
     *		water, and there's exactly one island (i.e.,  one or more connected
     *		land cells).
     *          The island doesn't have "lakes" (water inside that is not connected
     *		to the water around the island). One cell is a square with length 1.
     *		The grid is rectangular, width and height don't exceed 100.  Please
     *		determine the perimeter of the island.
     * Example:
     *	        Input: [[0,1,0,0], [1,1,1,0], [0,1,0,0], [1,1,0,0]]
     *          Output: 16
     * 		Explanation: The perimeter is the 16 yellow stripes as shown below
	...
	for (size_t i = 0; i < 4; ++i) {
	    ...
	    orientation[i] = std::make_pair(-static_cast<size_t>(std::sin(theta)),
	  				    static_cast<size_t>(std::cos(theta)));
	}

	// ============================= 初始版本 ============================= //
        // 最初是想利用BFS遍历grid，因为只有一个岛，这样可以省却很多不必要的遍历
        // 观察格点状态可知，有四个相邻1的点对周长没有贡献，有三个相邻1的贡献一条边，有
        // 两个相邻1的贡献两条边，而有一个相邻1的贡献三条边，没有相邻1的点为孤岛，将会
        // 产生四条边的贡献
        ...
        // // 用来处理岛屿只包含一个cell的情况
        // if (num_cross == 0 && num_corner == 0 && num_end == 0) return 4;
        // // 一般情况下从这里返回
        // return (num_cross + 2*num_corner + 3*num_end);

        // ============================= 改进版本 ============================= //
        // 仔细思考和观察可以发现:
        // (1) 其实上面的count数组是没有必要的存在，因为我对每个点计数完成后直接对其计
        // 数值进行判断，可以使用一个局部变量代替
        // (2) 相邻1的个数与贡献值可以用(4-count)公式来表达，不需要switch这样的结构
        int perimeter = 0;
        for (size_t i = 0; i < rows; ++i) {
            for (size_t j = 0; j < cols; ++j) {
                if (grid[i][j]==1) {
        ...
        for (size_t i = 0; i < rows; ++i){
            for (size_t j = 0; j < cols; ++j){
                if(grid[i][j] == 1){
                    ...
                    perimeter += 4*grid[i][j] - (north+west+south+east);
                }
            }
        }
