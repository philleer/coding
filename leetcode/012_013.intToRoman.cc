...
public:
	 ...
	 *	I, V, X, L, C, D and M, which means value 1, 5, 10, 50, 100, 500, 1000
	 * 	respectively.
	 * Note:
	 *	e.g., 2 is written as II in Roman numeral, two one's added together.
	 *	12 is written as, XII, which is simply X + II. Number 27 is written as
	 *	XXVII, which is XX + V + II.
	 *	Roman numerals are usually written largest to smallest from left to
	 *	right. However, the numeral for 4 is not IIII. Instead, it is written
	...
	int romanToInt(std::string s) {
		std::unordered_map<char, int> lut{{'I', 1}, {'V', 5}, {'X', 10},
						  {'L', 50}, {'C', 100}, {'D', 500},
						  {'M', 1000}};
	...
	std::string intToRoman(int num) {
		...
		std::vector<std::pair<char, int>> lut{{'I', 1}, {'V', 5}, {'X', 10},
						      {'L', 50}, {'C', 100}, {'D', 500},
						      {'M', 1000}};
		...
		for (int i = lut.size()-1; i >= 0; i -= 2) {
			...
			if (9==tmp || 4==tmp) {
				ret += lut[i].first;
				ret += (9==tmp ? lut[i+2].first : lut[i+1].first);
			} else if (tmp > 0) {
				if (tmp >= 5) {
					ret += lut[i+1].first;
					tmp -= 5;
