...
public:
	 ...
	 *	on the behavior of this function. If the first sequence of
	 *	non-whitespace characters in str is not a valid integral number,
	 *	or if no such sequence exists because either str is empty or it
	 ...
	 * 	[−2^31, 2^31 − 1]. If the numerical value is out of the range
	 * 	of representable values, INT_MAX (2^31 − 1) or INT_MIN (−2^31)
	 * 	is returned.
	 * Example:
	 *	Input: "42"		Output: 42
	 *
	 ...
	 *	is the minus sign.Then take as many numerical digits as
	 *	possible, it's 42.
	 *
	 ...
	 *	32-bit signed integer. Thefore INT_MIN (−231) is returned.
	 *=====================================================================
	 */
	int myAtoi1(std::string str) {
		...
		return (res > INT_MAX1 ? INT_MAX1 :
			(res < INT_MIN1 ? INT_MIN1 : (int)res));
	}
