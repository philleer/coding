...
public:
	...
	/*=====================================================================
	 * Description: There are two sorted arrays nums1 and nums2 of size m
	 * 	and n respectively. Find the median of the two sorted arrays.
	 * 	The overall run time complexity should be O(log(m+n)). You may
				      ...
				      std::vector<int> &nums2)
	{
		...
		double res = 0.0;
		int left = kth( &nums1[0], len1,
				&nums2[0], len2, (len1 + len2 + 1) / 2
			      );
		if ((len1 + len2) & 1) {
			res = (double)left;
		} else {
			int right = kth(&nums1[0], len1,
					&nums2[0], len2,
					(len1 + len2 + 1) / 2 + 1);
